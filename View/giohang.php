...
if($ok == 2){
   ...
   <?php
    ...
    $priceNew = (int)str_replace('.','',$row['priceNew']); // chuyển sô tiền khi 500.000 thành 500000
    $discount = (int)str_replace('.','',number_format($row['discount'],3)); // chuyển tiền về từ 70 thành 70000 thaeem3 số 0
    $total = $_SESSION['cart'][$customer_id][$row['id']]*($priceNew - $discount);

    ?>

    <form action="/Controller/xuLyDatHang.php" method="POST">
            ...
            <div class="cart__form">
                <div class="cart__form-product">
                    ...
                    <span class="cart__form-product__name" ><?php echo $row['title']?></span>
                      <!-- Thêm input hidden để gửi dữ liệu -->
                <input type="hidden" name="img" value="<?php echo $row['urlImg']; ?>">
                    ...
                    <div class="cart__form-productDetails">
                        ...
                        <div class="cart__form-productDetails__quantity-container">        
                            <div class="detail__quantity-content">
                                ...
                                <a href="/Controller/giamgiohang.php?item=<?php echo $row['id']?>" class="detail__quantity-content__minus" ><i class="fa-solid fa-minus detail__quantity-content__minus-icon" ></i></a>
                                    <div class="cart__form-productDetails__quantity-content__show--parent">
                                    <input type="text" class="cart__form-productDetails__quantity-content__show" name="qty['<?php echo $row['id']?>']" id="qty_id"  value="<?php echo $_SESSION['cart'][$customer_id][$row['id']]?>"></input>
                                    <input type="hidden" name="quantity" value="<?php echo $_SESSION['cart'][$customer_id][$row['id']]?>">

                                    </div>
                                <a href="/Controller/muangay.php?item=<?php echo $row['id']?>" class="detail__quantity-content__add" ><i class="fa-solid fa-plus"></i></a>              
                            </div>    
                        </div>
                        <span class="cart__form-productDetails__total" name="totalAmount" id="total-Price"><?php echo number_format($total,0).'đ'?></span>
                        <input type="hidden" name="totalAmount" value="<?php echo number_format($total,0).'đ'?>">
                        <a href="/Controller/xoagiohang.php?productid=<?php echo $row['id']?>" class="cart__form-productDetails__delete">Xóa</a>
                        <input type="hidden" name="productId" value="<?php echo $row['id']?>">
                </div>
            </div>
        </div> 
                <?php   
                $totalProduct +=$_SESSION['cart'][$customer_id][$row['id']]*($priceNew - $discount);                
                    }
                }
                    ?>
                <div class="cart__container-totalPrice">
                    <div class="cart__form-totalPrice">
                        ...
                        <!-- <a href="" class="cart__form-totalPrice--buy">Mua Ngay</a> -->
                         <input type="submit" name="submit" value="Mua Ngay" class="cart__form-totalPrice--buy">
                </div>
            </div>
    </form>
     <?php 
